import numpy as np
from scipy.signal import get_window

def prepare_cosine_filter(opt, T, Fs):
    """
    Compute the window and frequency filter matrix for FFT-based filtering.

    Parameters:
    - opt: dictionary with keys:
        - 'win': window type (string, e.g. 'boxcar', 'hann')
        - 'par': list of strings ('low', 'high', 'notch') for each filter
        - 'freq': list or array of center frequencies (in Hz)
        - 'width': list or array of filter bandwidths (in Hz)
    - T: length of the signal
    - Fs: sampling frequency (Hz)

    Returns:
    - win: window of length T
    - F: frequency-domain filter of length T
    """

    # MATLAB: win = feval(opt.win, T); → in Python, use scipy.signal.get_window
    win = get_window(opt['win'], T)  # applies the window function
    win = win.flatten()              # MATLAB win(:)' makes it a row vector

    # MATLAB: opt.freq/Fs*2 → normalize to Nyquist = 1
    norm_freqs = np.array(opt['freq']) / Fs * 2
    norm_widths = np.array(opt['width']) / Fs * 2

    # Apply cosine filter
    F = cos_filt(T, opt['par'], norm_freqs, norm_widths)
    return win, F


def cos_filt(quantum, par_list, f_vect, Ws_vect):
    """
    Generate cosine-shaped frequency filters.

    Parameters:
    - quantum: length of filter (same as signal length)
    - par_list: list of strings ('low', 'high', 'notch')
    - f_vect: normalized center frequencies (0 to 1)
    - Ws_vect: normalized bandwidths

    Returns:
    - F_h: frequency-domain filter
    """

    F_h = np.ones(quantum)  # MATLAB: F_h = ones(1, quantum)

    for k, (f, Ws) in enumerate(zip(f_vect, Ws_vect)):
        flow = quantum * (f - Ws / 2) / 2
        fhigh = quantum * (f + Ws / 2) / 2

        F_mult = np.ones(quantum)

        if par_list[k] == 'low':
            F_mult[:int(np.floor(flow)) + 1] = 1  # flat passband
            F_mult[int(np.ceil(fhigh)) + 1:int(np.ceil((quantum + 1) / 2))] = 0  # stopband

            # transition band
            trans_idx = np.arange(int(np.ceil(flow)) + 1, int(np.floor(fhigh)) + 2)
            F_mult[trans_idx] = np.cos((trans_idx - flow - 1) / (fhigh - flow) * (np.pi / 2))**2

        elif par_list[k] == 'high':
            F_mult[:int(np.floor(flow)) + 1] = 0
            F_mult[int(np.ceil(fhigh)) + 1:int(np.ceil((quantum + 1) / 2))] = 1

            trans_idx = np.arange(int(np.ceil(flow)) + 1, int(np.floor(fhigh)) + 2)
            F_mult[trans_idx] = np.sin((trans_idx - flow - 1) / (fhigh - flow) * (np.pi / 2))**2

        elif par_list[k] == 'notch':
            F_mult[:int(np.floor(flow)) + 1] = 1
            F_mult[int(np.ceil(fhigh)) + 1:int(np.ceil((quantum + 1) / 2))] = 1

            trans_idx = np.arange(int(np.ceil(flow)) + 1, int(np.floor(fhigh)) + 2)
            F_mult[trans_idx] = np.cos((trans_idx - flow - 1) / (fhigh - flow) * np.pi)**2

        else:
            raise ValueError("The filter must be 'low', 'high', or 'notch'")

        # Mirror the first half of the filter to the second half
        F_mult[int(np.ceil((quantum + 1) / 2)):] = F_mult[int(np.floor((quantum + 1) / 2)) - 1::-1]

        # Multiply into cumulative filter
        F_h *= F_mult

    return F_h
